...
v2 Changes:
- Reports ordered by patient index (extracted from filename suffix)
- Separate workflow for non-abdomen cases (findings + impression only)
"""
...
# Abdomen ultrasound types - these use the specialized multi-agent workflow
ABDOMEN_TYPES = {'abdomen', 'liver', 'kidney', 'hepatobiliary', 'renal', 'hbs'}


def is_abdomen_case(ultrasound_type: str) -> bool:
    """Determine if the ultrasound type should use the abdomen workflow"""
    if not ultrasound_type:
        return True  # Default to abdomen
    return ultrasound_type.lower().strip() in ABDOMEN_TYPES
...
def extract_patient_index(filename: str) -> int:
    """
    Extract patient index from filename.
    Filename format: e.g., "12jun1.pdf", "12jun2.pdf", "13jun1.pdf"
    Returns the trailing number (patient index).
    """
    # Remove .pdf extension if present
    name = filename.replace('.pdf', '').replace('.PDF', '')
    
    # Find the trailing number (patient index)
    # Pattern: letters/numbers followed by trailing digits
...
def extract_number(filename):
    """Extract number from filename for sorting (legacy - kept for compatibility)"""
...
def read_pdfs_in_folder(folder_path):
    ...
    Returns a list of patient data dictionaries with patient_index for ordering.
    """
    patients = []
    ...
    # Get all PDF files and sort by patient index
    pdf_files = [f for f in os.listdir(folder_path) if f.endswith('.pdf') or f.endswith('.PDF')]
    pdf_files.sort(key=extract_patient_index)
    
    for file_name in pdf_files:
        file_path = os.path.join(folder_path, file_name)
        patient_index = extract_patient_index(file_name)
        ...
        try:
            ...
            print(f"    ✓ Read {file_name} (Index: {patient_index}, Type: {ultrasound_type})")
...
def process_date_folders(base_folder_path):
    ...
    Returns a dictionary mapping dates to list of patient data (ordered by patient index).
    """
    date_findings = {}
    ...
    for date_folder in subdirs:
        ...
        # Read PDFs from this date folder (returns list ordered by patient index)
        patients = read_pdfs_in_folder(date_folder_path)
        
        if patients:
            date_findings[date_folder] = patients
            
            # Count abdomen vs non-abdomen cases
            abdomen_count = sum(1 for p in patients if is_abdomen_case(p['ultrasound_type']))
            non_abdomen_count = len(patients) - abdomen_count
            
            print(f"  ✓ Found {len(patients)} patient(s)")
            print(f"    - Abdomen cases: {abdomen_count}")
            print(f"    - Non-abdomen cases: {non_abdomen_count}")
        else:
...
class SplitterAgent(BaseAgent):
    ...
    def __init__(self, api_key: str):
        ...
        self.system_prompt = """You are a medical data extraction specialist. Extract information from radiology patient reports and organize by body part. Make sure to split accurately and do not repeat information.
...
* Do not miss out any important findings
--- EDGE CASES
...
- Use full stops at the end of each line

- Focus only on clinically significant findings (Exclude normal findings and previous comparison statements)

- Do not mention "Focal Spared Area"

- If the entire ultrasound is normal, impression should only be "Unremarkable ultrasound study. """


# ============================================================================
# NON-ABDOMEN WORKFLOW AGENTS
# ============================================================================

class NonAbdomenFindingsAgent(BaseAgent):
    """
    Agent for generating findings section for non-abdomen ultrasound cases.
    Handles: Thyroid, Breast, Pelvis, Scrotum, Soft Tissue, Musculoskeletal, etc.
    """
    
    def __init__(self, api_key: str):
        super().__init__(api_key)
        self.system_prompt = """You are a radiologist generating the FINDINGS section for a non-abdominal ultrasound report.

Your job: Generate a professional, concise findings section from the provided examination data.
Output only the report text — no headings, no metadata, no explanations, and no extra commentary.

--- GENERAL RULES:
1. Begin with a statement about the organ/region being examined and its overall appearance (normal, enlarged, heterogeneous, etc.)
2. Describe any focal findings with:
   - Exact location (laterality, region within organ)
   - Lesion type (nodule, cyst, mass, collection, etc.)
   - Size measurements (preserve decimal precision)
   - Characteristics (echogenicity, margins, vascularity, calcifications, etc.)
   - Any qualifiers provided (suspicious, benign-appearing, indeterminate, etc.)
3. For multiple similar findings, group them logically and mention the largest/most significant
4. Include comparison to prior studies if mentioned
5. State if any structures are not well visualized
6. End with an appropriate absence statement if no suspicious findings are present

--- SPECIFIC ORGAN GUIDELINES:

THYROID:
- State overall thyroid appearance and size if provided
- Describe nodules with location (right/left lobe, isthmus), size, composition (solid/cystic/mixed), echogenicity, margins, calcifications
- Use TI-RADS terminology if provided in input
- "No suspicious thyroid nodule is identified." for normal/benign findings

BREAST:
- State breast composition if provided (fibroglandular, fatty, heterogeneous)
- Describe masses with clock position, distance from nipple, size, shape, margins, echogenicity
- Use BI-RADS terminology if provided in input
- Include axillary lymph node status

PELVIS (Female):
- Describe uterus (size, position, myometrium, endometrium)
- Describe ovaries (size, follicles, any masses/cysts)
- Note free fluid if present
- "No adnexal mass is seen." for normal findings

SCROTUM/TESTICULAR:
- Describe each testis (size, echogenicity, any lesions)
- Describe epididymis
- Note presence of hydrocele, varicocele, or hernias
- Include Doppler findings if provided

SOFT TISSUE/MUSCULOSKELETAL:
- Describe location and extent of abnormality
- Note relationship to surrounding structures
- Describe any collections, masses, or tears
- Include measurements and characteristics

NECK (non-thyroid):
- Describe lymph nodes (location, size, morphology, hilum)
- Note any masses or collections
- Include salivary gland findings if examined

--- MEASUREMENT FORMAT:
- Two/three-dimensional: A x B mm or A x B x C mm
- Preserve decimal precision as provided
- For multiple measurements, use commas to separate

--- FINAL INSTRUCTION:
When you receive the examination data, produce only the findings paragraph(s) adhering to the guidelines above.
Do not output anything else — no commentary, no bullet lists, no headers, and no surrounding quotes."""


class NonAbdomenImpressionAgent(BaseAgent):
    """
    Agent that generates the IMPRESSION section for non-abdomen cases.
    """
    
    def __init__(self, api_key: str):
        super().__init__(api_key)
        self.system_prompt = """You are a radiologist creating the IMPRESSION section of a non-abdominal ultrasound report.

The impression should:
- Summarize the most significant findings concisely
- Use terminology consistent with the findings section
- Avoid filler words (e.g., "a," "is," "are," "noted," "present," "seen")
- Use short, direct noun phrases or descriptors
- Separate each finding with a new line (no bullets or numbering)
- Use full stops at the end of each line
- Focus only on clinically significant findings
- Exclude normal findings unless the entire study is unremarkable
- Include appropriate classification (TI-RADS, BI-RADS) if provided in findings

For unremarkable studies, use:
- Thyroid: "Unremarkable thyroid ultrasound."
- Breast: "Unremarkable breast ultrasound." or "BI-RADS 1: Negative."
- Pelvis: "Unremarkable pelvic ultrasound."
- Scrotum: "Unremarkable scrotal ultrasound."
- Other: "Unremarkable [organ/region] ultrasound."

Do not add recommendations or follow-up suggestions unless explicitly provided in the input."""


# ============================================================================
# CENTRAL AGENT - ROUTES TO APPROPRIATE WORKFLOW
# ============================================================================

class CentralAgent:
    """Central coordinator that manages all specialized agents and routes to appropriate workflow"""
    
    def __init__(self, api_key: str):
        # Abdomen workflow agents
        ...
        # Non-abdomen workflow agents
        self.non_abdomen_findings_agent = NonAbdomenFindingsAgent(api_key)
        self.non_abdomen_impression_agent = NonAbdomenImpressionAgent(api_key)
    
    async def process_patient_async(self, patient_data: str, ultrasound_type: str = "Abdomen") -> str:
        """Process a single patient's data asynchronously - routes to appropriate workflow"""
        
        if is_abdomen_case(ultrasound_type):
            return await self._process_abdomen_case(patient_data, ultrasound_type)
        else:
            return await self._process_non_abdomen_case(patient_data, ultrasound_type)
    
    async def _process_abdomen_case(self, patient_data: str, ultrasound_type: str) -> str:
        """Process abdomen case using the specialized multi-agent workflow"""
...
"""
    ...
    async def _process_non_abdomen_case(self, patient_data: str, ultrasound_type: str) -> str:
        """Process non-abdomen case using the simplified findings + impression workflow"""
        
        print(f"\n[NON-ABDOMEN WORKFLOW] Processing {ultrasound_type} case...")
        
        # Step 1: Generate findings
        print("\n[1] Generating findings section...")
        findings_prompt = f"""Generate a professional findings section for this {ultrasound_type} ultrasound examination:

EXAMINATION DATA:
{patient_data}

ULTRASOUND TYPE: {ultrasound_type}

Provide only the findings text, no headers or labels."""
        
        findings = await self.non_abdomen_findings_agent.generate_response_async(
            findings_prompt,
            self.non_abdomen_findings_agent.system_prompt
        )
        
        # Step 2: Generate impression
        print("\n[2] Generating impression section...")
        impression_prompt = f"""Based on these findings, generate a professional IMPRESSION section:
...
ORIGINAL EXAMINATION DATA:
{patient_data}

Provide only the impression text, no headers."""
        
        impression = await self.non_abdomen_impression_agent.generate_response_async(
            impression_prompt,
            self.non_abdomen_impression_agent.system_prompt
        )
...
# ============================================================================
# RADIOLOGY REPORT GENERATOR - MAIN APPLICATION CLASS
# ============================================================================
...
class RadiologyReportGenerator:
    ...
    async def process_batch_async(self, patient_data_list: List[Dict], date: str) -> str:
        """Process multiple patients concurrently, ordered by patient index"""
        print(f"\n{'='*80}")
        print(f"PROCESSING BATCH FOR DATE: {date} (ASYNC MODE)")
        print(f"Total patients: {len(patient_data_list)}")
        print(f"{'='*80}")
        
        # Sort patients by patient_index to ensure correct order
        sorted_patients = sorted(patient_data_list, key=lambda x: x.get('patient_index', 0))
        
        # Separate into abdomen and non-abdomen cases for logging
        abdomen_cases = [p for p in sorted_patients if is_abdomen_case(p.get('ultrasound_type', 'Abdomen'))]
        non_abdomen_cases = [p for p in sorted_patients if not is_abdomen_case(p.get('ultrasound_type', 'Abdomen'))]
        
        print(f"\n  Workflow Distribution:")
        print(f"    - Abdomen cases: {len(abdomen_cases)}")
        print(f"    - Non-abdomen cases: {len(non_abdomen_cases)}")
        
        # Process all patients concurrently
        print(f"\n⚡ Processing {len(sorted_patients)} patients in parallel...")
        start_time = time.time()
        
        tasks = []
        patient_info = []  # Store (name, index, type) for each patient
        
        for i, patient_data in enumerate(sorted_patients, 1):
            examination_finding = patient_data.get('examination_finding', '')
            ultrasound_type = patient_data.get('ultrasound_type', 'Abdomen')
            name = patient_data.get('name', f'Unknown_{i}')
            patient_idx = patient_data.get('patient_index', i)
            workflow = "ABDOMEN" if is_abdomen_case(ultrasound_type) else "NON-ABDOMEN"
            
            print(f"  • Queuing Patient {i}/{len(sorted_patients)}: {name} (Index: {patient_idx}, Type: {ultrasound_type}, Workflow: {workflow})")
            
            patient_info.append({
                'name': name,
                'index': patient_idx,
                'type': ultrasound_type
            })
        ...
        print(f"\n✓ All {len(sorted_patients)} patients processed in {elapsed:.2f} seconds!")
        print(f"  Average: {elapsed/len(sorted_patients):.2f} seconds per patient")
        
        # Format reports in order (already sorted by patient_index)
        all_reports = []
        for i, (report, info) in enumerate(zip(reports, patient_info), 1):
            header = f"{'='*80}\nPATIENT {info['name']} (Index: {info['index']}, Type: {info['type']})\n{'='*80}"
            all_reports.append(f"{header}\n\n{report}")
        ...
        print(f"✓ Reports ordered by patient index (1, 2, 3, ...)")
        print(f"{'='*80}\n")
...
def main():
    ...
    print("""
╔══════════════════════════════════════════════════════════════╗
║      MULTI-AGENT RADIOLOGY REPORT GENERATOR v2               ║
║      Powered by Gemini 2.0 Flash                             ║
║      with Google Drive Integration                           ║
║                                                              ║
║      NEW FEATURES:                                           ║
║      • Reports ordered by patient index                      ║
║      • Separate workflow for non-abdomen cases               ║
╚══════════════════════════════════════════════════════════════╝
    """)
    ...
    try:
        ...
        for date, patient_list in date_findings.items():
            print(f"\n{'='*80}")
            print(f"📅 DATE: {date}")
            print(f"{'='*80}")
            
            # patient_list is already ordered by patient_index from read_pdfs_in_folder
            print(f"\n  Total examinations for {date}: {len(patient_list)}")
            
            # Show patient order
            print(f"\n  Patient Order:")
            for p in patient_list:
                workflow = "ABDOMEN" if is_abdomen_case(p['ultrasound_type']) else "NON-ABDOMEN"
                print(f"    {p['patient_index']}. {p['name']} ({p['ultrasound_type']}) → {workflow} workflow")
            
            if patient_list:
                output_file = generator.process_batch(patient_list, date)
